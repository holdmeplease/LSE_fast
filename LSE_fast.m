...
while num_U ~= 0
    ...
    %寻找下一个查询点
    [point_query(t, :), point_idx] = SelectPoint(U_new, num_U_new, C_new, h);
    %带有观测噪声的观测结果
    answer_query(t) = Sinusoidal2D(point_query(t,:)) + sigma_noise * randn(1); 
    
    %改进部分
    if answer_query(t) > h %查询点立即分类
        num_H = num_H + 1;
        H(num_H, :) = point_query(t, :);
    else
        num_L = num_L + 1;
        L(num_L, :) = point_query(t, :);
    end
    U_new(point_idx, :) = U_new(num_U_new, :); %将查询点从U中移除，同时维护U和C的对应关系
    C_new(point_idx, :) = C_new(num_U_new, :);
    %改进部分
    
    num_U_new = num_U_new - 1;
    U = U_new; %更新未分类点集合
    num_U = num_U_new;
    C = C_new; %更新未分类点的置信区间
    %计算所有未分类点的u和segma
    [u, sigma] = UpdateGP(t, point_query, answer_query, sigma_noise, U, num_U); 
